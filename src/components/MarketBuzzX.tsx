...
import { MessageCircle, Heart, ExternalLink, Loader2, ChevronRight, RefreshCw } from 'lucide-react';
...
export default function MarketBuzzX() {
  ...
  const fetchPosts = useCallback(async (isRefresh = false, clearError = true) => {
    if (isRefresh) setIsRefreshing(true);
    if (clearError) setRefreshError(null);
  ...
  const triggerRefresh = useCallback(async (showErrors = true) => {
    if (isRefreshInProgress) return;
    
    setIsRefreshInProgress(true);
    setIsRefreshing(true);
    if (showErrors) setRefreshError(null);
    
    try {
      const refreshRes = await fetch('/api/social/refresh', {
        method: 'POST',
        credentials: 'include',
      });
      ...
      if (refreshRes.ok) {
        ...
        console.log('X refresh result:', data);
        await fetchPosts(true, true);
      } else {
        if (showErrors) {
          const errorData = await refreshRes.json();
          if (refreshRes.status === 403) {
            setRefreshError('Premium feature');
          } else if (refreshRes.status === 401) {
            setRefreshError('Sign in required');
          } else {
            setRefreshError(errorData.error || 'Refresh failed');
          }
        }
        await fetchPosts(true, !showErrors);
      }
    } catch (error) {
      console.error('Refresh error:', error);
      if (showErrors) setRefreshError('Network error');
      await fetchPosts(true, !showErrors);
    } finally {
      setIsRefreshInProgress(false);
    }
  }, [fetchPosts, isRefreshInProgress]);
  ...
  useEffect(() => {
    ...
    if (isPremium) {
      const checkAndRefresh = async () => {
        try {
          const statusRes = await fetch('/api/social/refresh', { credentials: 'include' });
          if (statusRes.ok) {
  ...
  }, [statusResolved, isPremium, fetchPosts, triggerRefresh]);
      ...
      <div className="p-6 border-b border-gray-200 dark:border-slate-700">
        ...
        <div className="flex items-center justify-between mt-1">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Live updates from Nigerian finance sources
          </p>
          <div className="flex items-center gap-2">
            {refreshError && (
              <span className="text-xs text-amber-500">{refreshError}</span>
            )}
            {lastUpdated && (
      ...
      <div className="p-4">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-12 gap-2">
            ...
            <span className="text-sm text-gray-500">Loading market buzz...</span>
          </div>
        ) : posts.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-3">No market buzz available</p>
            <button
              onClick={() => triggerRefresh(true)}
              disabled={isRefreshing}
